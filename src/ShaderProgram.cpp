...
check_program_status(GLuint program)
{
  ...
  if (info_log_length > 1)
  {
      ...
      glGetProgramInfoLog(program, info_log_length, &chars_written, &info_log[0]);
...
compile_shader( const std::string& gpu_name, GLuint type )
{
  ...
  if ( !gpu_file.is_open() )
    {
      std::cerr << "cannot open shader file " << gpu_name << ". Are you in the right directory?";
      return 0;
    }
  ...
  if ( !shader )
    {
      std::cerr << "error when creating shader [" << gpu_name << "]";
      return 0;
    }
  ...
  if( GL_FALSE == result )
    {
      std::cerr << "shader [" << gpu_name << "] compilation failed!";
    ...
    const std::string& fragment_file_path )
{
  ...
  if( !vertex_shader_id || !fragment_shader_id )
    {
      std::cerr << "cannot load shader program. Program unchanged...";
  ...
  if( check_program_status(m_programId) )
    {
      ...
      if( glIsProgram( previous_id ) )
        glDeleteProgram( previous_id );
      m_vertexFilename = vertex_file_path;
      m_fragmentFilename = fragment_file_path;

      // load attributes and uniforms
      std::cout << "resources info for ShaderProgram "<< this << " (" << vertex_file_path << ", " << fragment_file_path << ")";
      resources_introspection();
    }
  // it failed: delete new program and restore to previous state
  else
    {
      std::cout << "shader program described by (" << vertex_file_path << ", " << fragment_file_path
...
void ShaderProgram::resources_introspection()
{
  ...
  std::cout << " * uniforms: " << num_uniforms;
  ...
  for(int unif = 0; unif < num_uniforms; ++unif)
  {
    ...
    std::cout << "\t" << std::setw(8) << values[2] << "   " << name;
  ...
  std::cout <<  " * attributes: " << num_attributes;
  std::cout <<  "\tLocation   Name";
  for( int att = 0; att < num_attributes; ++ att )
    {
      glGetProgramResourceiv( m_programId, GL_PROGRAM_INPUT, att, 2, attribute_properties, 2, NULL, values );
      char* name = new char[values[0]];
      glGetProgramResourceName(m_programId, GL_PROGRAM_INPUT, att, values[0], NULL, &name[0]);
      std::cout << "\t" << std::setw(8) << values[1] << "   " << name;
