...
class Collided {
	public:
		...
		const glm::vec3 getOverlapVector(const CollisablePtr<Cobble> cobble1, const CollisablePtr<Cobble> cobble2) {
			...
			//overlap.x = glm::max(0.f, glm::min(cobble1->m_position.x + cobble1->m_size.x, cobble2->m_position.x + cobble2->m_size.x) - glm::max(cobble1->m_position.x, cobble2->m_position.x));
  			//overlap.z = glm::max(0.f, glm::min(cobble1->m_position.z + cobble1->m_size.z, cobble2->m_position.z + cobble2->m_size.z) - glm::max(cobble1->m_position.z, cobble1->m_position.z));
